...
- 메인에서 바로가기 섹션으로 아래 DB/페이지 접근 가능
...
> 참고: `Todo - 사용자님`은 오세영과 중복이라 아카이브 처리됨
...
2. 장비 이슈 발생 시
   ...
   - 담당자 Todo 생성/연결
3. 링크 연결 규칙
   - 이슈의 `연결 Todo URL` ↔ Todo의 `연결 이슈 URL` 상호 기입
4. 상태 동기화 원칙
   - 이슈 완료 시 Todo 완료 처리
